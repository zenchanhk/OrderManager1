...
class Utils {
    ...
    private static final URL BaseDir = IBController.class.getProtectionDomain().getCodeSource().getLocation();
    /**
    ...
    static String getBaseDir() {
        try {
            File f = new File(Utils.BaseDir.toURI());
            return f.getParent();
        } catch (Exception ex) {
    ...
    static void logToFile(String msg){
        String fileName = "";
        try {
            fileName = Paths.get(getBaseDir(), "log.log").toString();
