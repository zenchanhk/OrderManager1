...
class SwingUtils {
     ...
     *  the Component, if it was found;  otherwise null
     */
    static Component findComponentByClass(Container container, String text) {
        ComponentIterator iter = new ComponentIterator(container);
        while (iter.hasNext()) {
            Component component = iter.next();
            if (text.equals(component.getClass().getName())) return component;
        }
        return null;
    }

    static String[] captureComponent(Component component) {
        Rectangle rect = component.getBounds();

        try {
            String base_dir = Utils.getBaseDir();
            String format = "png";

            String fileName1 = Paths.get(base_dir, "images", System.currentTimeMillis() + "1." + format).toString();
            String fileName2 = Paths.get(base_dir, "images", System.currentTimeMillis() + "2." + format).toString();
            BufferedImage captureImage =
                    new BufferedImage(rect.width, rect.height, BufferedImage.TYPE_INT_ARGB);
            component.paint(captureImage.getGraphics());
            BufferedImage img1 = captureImage.getSubimage(captureImage.getWidth()/4,0,captureImage.getWidth()/4,captureImage.getHeight());
            BufferedImage img2 = captureImage.getSubimage(captureImage.getWidth()/2,0,captureImage.getWidth()/4,captureImage.getHeight());
            //ImageIO.write(captureImage, format, new File(fileName));
            ImageIO.write(img1, format, new File(fileName1));
            ImageIO.write(img2, format, new File(fileName2));
            //System.out.printf("The screenshot of %s was saved!", component.getName());
            return new String[] {fileName1, fileName2};
        } catch (Exception ex) {
            System.err.println(ex.getMessage());
            Utils.logToFile(ex.getMessage());
            return null;
        }
    }
    /**
    ...
    static boolean findTextArea(Container container, String content) {
        ComponentIterator iter = new ComponentIterator(container);
        int i = 0;
        while (iter.hasNext()) {
            Component component = iter.next();
            if (component instanceof JTextArea &&
                    ((JTextArea)component).getText().contains(content))
                return true;
        }
        return false;
    }

    static String getTextAreaContent(Container container, int ith) {
        ComponentIterator iter = new ComponentIterator(container);
        int i = 0;
        while (iter.hasNext()) {
            Component component = iter.next();
            if (component instanceof JTextArea && i++ == ith)
...
/**
    ...
    private static final String simpleMlp = Paths.get(Utils.getBaseDir(), "captcha_model.hdf5").toString();
    ...
    static String captchaDecoder(String img_path) {
        try {
            ...
            Mat img1 = new Mat();
            Mat thresh = new Mat();
            //Core.copyMakeBorder(img, img1, 20, 20, 20, 20, Core.BORDER_REPLICATE);
            ...
            // sort regions
            Collections.sort(regions, (o1, o2) -> Integer.compare(o1.x, o2.x));
