...
namespace Krs.Ats.IBNet
{
    ...
    public class IBClient : IDisposable
    {
        ...
        {
            RaiseEvent(Position, this, e);
        }

        private void position(string account, ContractDetails contract, double pos, double avgCost)
        {
            var e = new PositionEventArgs(account, contract, pos, avgCost);
            OnPosition(e);
        }

        /// <summary>
        /// Called on a PositionEnd call back.
        /// </summary>
        public event EventHandler<EventArgs> PositionEnd;

        /// <summary>
        /// Called internally when a Position Event ends.
        /// </summary>
        protected virtual void OnPositionEnd(EventArgs e)
        {
            RaiseEvent(PositionEnd, this, e);
        }
        ...
        /// <summary>
        /// Called on a ConnectionStatus call back.
        /// </summary>
        public event EventHandler<ConnectionStatusEventArgs> ConnectionStatus;

        /// <summary>
        /// Called internally when the receive thread receives a Connection Status Event.
        /// </summary>
        protected virtual void OnConnectionStatus(ConnectionStatusEventArgs e)
        {
            RaiseEvent(ConnectionStatus, this, e);
        }

        private void connectionStatus(bool isConnected)
        {
            var e = new ConnectionStatusEventArgs(isConnected);
        ...
        private void error(int tickerId, ErrorMessage errorCode, string errorMsg)
        {
            lock (this)
            {
                ...
                if (errorCode == ErrorMessage.ClientIdAlreadyInUse)
                    close();
            }
        }
        ...
        /// <summary>
        /// Call this function to requests real-time position data for all accounts.
        /// </summary>
        public void RequestPositions()
        {
            lock (this)
            {
                ...
                // send request positions msg
                try
                {
                    send((int)OutgoingMessage.RequestPositions);
                    send(version);
                }
                catch (Exception e)
                {
                    ...
                    error(ErrorMessage.FailSendRequestPositions, e);
                    close();
                }
            }
        }
        ...
        public void CancelPositions()
        {
            lock (this)
            {
                ...
                try
                {
                    send((int)OutgoingMessage.CancelPositions);
                    send(version);
                }
                catch (Exception e)
                {
                    ...
                    error(ErrorMessage.FailSendCancelPositions, e);
                    close();
                }
            }
        }
        ...
        /// 
        /// TWS orders can only be bound to clients with a clientId of ??
        /// </summary>
        ...
        private bool ProcessMsg(IncomingMessage msgId)
        {
            ...
            switch (msgId)
            {
                case IncomingMessage.Undefined:
                    break;
                ...
                case IncomingMessage.Position:
                    {
                        ...
                        double pos = (serverVersion >= 101) ? ReadDouble() : ((double)ReadInt());
